...
type Provider interface {
	AllowOutOfBoundsSymlinks() (bool, error)
	AllowedNodeLabels() ([]string, error)
	AllowedScmProviders() ([]string, error)
	AppInstanceLabelKey() (string, error)
	ApplicationNamespaces() ([]string, error)
	BaseHRef() (string, error)
	CMPTarExcludedGlobs() ([]string, error)
	CMPUseManifestGeneratePaths() (bool, error)
	CommitAuthorEmail() (string, error)
	CommitAuthorName() (string, error)
	ContentSecurityPolicy() (string, error)
	ContentTypes() ([]string, error)
	DexServerAddr() (string, error)
	DexServerPlaintext() (bool, error)
	DexServerStrictTLS() (bool, error)
	DisableAuth() (bool, error)
	DisableHelmManifestMaxExtractedSize() (bool, error)
	DisableOCIManifestMaxExtractedSize() (bool, error)
	EnableBuiltinGitConfig() (bool, error)
	EnableGZip() (bool, error)
	EnableGitHubAPIMetrics() (bool, error)
	EnableK8sEvent() ([]string, error)
	EnableNewGitFileGlobbing() (bool, error)
	EnableProxyExtension() (bool, error)
	EnableScmProviders() (bool, error)
	EnabledSourceTypes() (map[string]bool, error)
	GitRequestTimeout() (time.Duration, error)
	GitSubmoduleEnabled() (bool, error)
	HardReconciliationTimeout() (time.Duration, error)
	HelmChartCacheExpiration() (time.Duration, error)
	HelmManifestMaxExtractedSize() (int64, error)
	HelmRegistryMaxIndexSize() (int64, error)
	HelmSettings() (*v1alpha1.HelmOptions, error)
	HelmUserAgent() (string, error)
	HydratorEnabled() (bool, error)
	...
	IncludeHiddenDirectories() (bool, error)
	Insecure() (bool, error)
	...
	ListenHost() (string, error)
	ListenPort() (int, error)
	MaxCombinedDirectoryManifestsSize() (resource.Quantity, error)
	MetricsClusterLabels() ([]string, error)
	MetricsHost() (string, error)
	MetricsPort() (int, error)
	OCIManifestMaxExtractedSize() (int64, error)
	OCIMediaTypes() ([]string, error)
	ParallelismLimit() (int64, error)
	PauseGenerationAfterFailedGenerationAttempts() (int, error)
	PauseGenerationOnFailureForMinutes() (int, error)
	PauseGenerationOnFailureForRequests() (int, error)
	PersistResourceHealth() (bool, error)
	ReconciliationJitter() (time.Duration, error)
	ReconciliationTimeout() (time.Duration, error)
	RepoCacheExpiration() (time.Duration, error)
	...
	RevisionCacheExpiration() (time.Duration, error)
	RevisionCacheLockTimeout() (time.Duration, error)
	RootPath() (string, error)
	ScmRootCAPath() (string, error)
	...
	StaticAssetsDir() (string, error)
	StreamedManifestMaxExtractedSize() (int64, error)
	StreamedManifestMaxTarSize() (int64, error)
	SubmoduleEnabled() (bool, error)
	...
	SyncWithReplaceAllowed() (bool, error)
	TrackingMethod() (string, error)
	Unsubscribe(subCh chan<- *settings.ArgoCDSettings)
	WebhookParallelism() (int, error)
	WebhookRefreshWorkers() (int, error)
	XFrameOptions() (string, error)
}
