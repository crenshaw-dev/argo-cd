...
func TestCertsAreNotGeneratedInInsecureMode(t *testing.T) {
	...
	defer closer()
	insecure, err := s.ConfigProvider().Insecure()
	require.NoError(t, err)
...
connectors:
  ...
  - type: OIDC
	...
	argocd = NewServer(t.Context(), argoCDOpts, ApplicationSetOpts{})
	var err error
	dexServerAddr, err := argocd.ConfigProvider().DexServerAddr()
	require.NoError(t, err)
	baseHRef, err := argocd.ConfigProvider().BaseHRef()
	require.NoError(t, err)
...
func Test_StaticAssetsDir_no_symlink_traversal(t *testing.T) {
	...
	staticAssetsDir, err := argocd.ConfigProvider().StaticAssetsDir()
	require.NoError(t, err)
